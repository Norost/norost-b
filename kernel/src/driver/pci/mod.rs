...
use crate::util::{u16le, u32le, VolatileCell};
use acpi::{AcpiTables, AcpiHandler, PciConfigRegions};
...
impl<'a> Header<'a> {
	...
	pub unsafe fn from_raw(address: *const Page) -> Self {
	...
	H: AcpiHandler
{
	...
	let pci = PCI::new(virt, phys.try_into().unwrap(), size, &[]);
